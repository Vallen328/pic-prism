  ...
  return (
    <div>
      ...
      <div className="w-[83vw] sm:w-[80vw] ml-8 p-2 bg-white rounded-2xl shadow-md flex flex-col justify-between items-center gap-5">
        ...
        <p>Total Earned {pathname == "/seller/profile" ? "Earned" : "Spent"} : ${pathname == "/seller/profile" ? calculateTotalForSeller(thisYear) : calculateTotalForBuyer(thisYear)}</p>
      </div>

      {/* My 3 expense card will be here */}
        {
          ...
          <ExpenseCard data = {thisWeek} title = {`${pathname == "/seller/profile" ? "Earned" : "Spent"} This Week`} dataKey = "price" value = {pathname == "/seller/profile" ? calculateTotalForSeller(thisWeek) : calculateTotalForBuyer(thisWeek)} />
          <ExpenseCard data = {thisMonth} title = {`${pathname == "/seller/profile" ? "Earned" : "Spent"} This Month`} dataKey = "price" value = {pathname == "/seller/profile"  ? calculateTotalForSeller(thisMonth) : calculateTotalForBuyer(thisMonth)} />
          <ExpenseCard data = {tillNow} title = {`${pathname == "/seller/profile"  ? "Earned" : "Spent"} Till Now`} dataKey = "price" value = {pathname == "/seller/profile" ? calculateTotalForSeller(tillNow) : calculateTotalForBuyer(tillNow)} />
        </div>
    )} 
    </div>
  )}
