...
const login = async (req, res) => {
    ...
    try{
        ...
        if(!comparePassword) return res.status(400).json({success: false, message: "Ivalid Credentials"});
        const data = {
            id: user._id,
...
const refresh = async(req, res) => {
    ...
    try{
        jwt.verify(token, process.env.REFRESH_TOKEN_SECRET, async (err, user) => {
            ...
            return res.status(200).json({success: true, message: "Token refreshed successfully", accessToken, refreshToken, role: user.accountType, author: user.author,})
        });
    }catch(error){
        return res.status(500).json({success: false, message: error.message});
    }
...
const switchProfile = async(req, res) => {
    ...
    try{
        ...
        return res.status(200).json({success: true, message: `Switched to ${user.accountType}`, accessToken, refreshToken, role: user.accountType, author: user.username});    
    }catch(error){
        return res.status(500).json({success: false, message: error.message});
    }
}
